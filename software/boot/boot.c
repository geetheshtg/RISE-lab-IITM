...
void BootfromAxiExp(long int* tcm_address){
   ...
   int valueL = (value&0xFFFFFFFF);
   int valueU = ((value >> 32)&0xFFFFFFFF);
   if(valueL!=0xdeadbeef){
       printf("\tBootMgr: Check Strings Don't match Exiting. Reset and Try Again\n");  //Looping for now, could actually boot from UART
       while(1);
   }
   else{
       AxiExpBaseAddr++;
       for(int i = 0; i < valueU; ++i){
